...
def get_sam_model_output(sam, batched_input, prompt_mask, it=1):
    # prompt_mask: [B,1,256,256] in {0,1}
    ...
    sam_mask = (sam_mask - sam_mask.min()) / (sam_mask.max() - sam_mask.min() + 1e-6)
...
def zero_sam_with_points(sam, batched_input, p_i, n_i):
    ...
    pos_points = []
    neg_points = []
    # image_size: (H', W') for the transformed input
    Hp, Wp = batched_input[0]['image_size']
    sx = float(Wp) / 64.0
    sy = float(Hp) / 64.0
    for i in range(pos):
        xg = (p_i[i] % 64).float() + 0.5
        yg = (p_i[i] // 64).float() + 0.5
        pos_points.append([xg * sx, yg * sy])
    for i in range(neg):
        xg = (n_i[i] % 64).float() + 0.5
        yg = (n_i[i] // 64).float() + 0.5
        neg_points.append([xg * sx, yg * sy])
    if len(pos_points)==0:
        pos_points = torch.empty((0,2))
    else:
        pos_points = torch.stack([torch.tensor(p) for p in pos_points], dim=0)
    if len(neg_points)==0:
        neg_points = torch.empty((0,2))
    else:
        neg_points = torch.stack([torch.tensor(p) for p in neg_points], dim=0)
    labels = torch.cat((neg_labels,pos_labels)).cuda().unsqueeze(dim=1)
    points = torch.cat((neg_points,pos_points), dim=0).cuda().unsqueeze(dim=1)
    input_images = torch.stack([sam.preprocess(x["image"].cuda()) for x in batched_input], dim=0)
    image_embeddings = sam.image_encoder(input_images)
...
def zero_sam_with_boxes(sam, batched_input, box_xyxy):
    # box_xyxy: torch.tensor shape [1,4] in pixel coords of resized input (image_size)
    if not isinstance(box_xyxy, torch.Tensor):
        box_xyxy = torch.tensor(box_xyxy, dtype=torch.float32)
    if box_xyxy.ndim == 1:
        box_xyxy = box_xyxy.unsqueeze(0)
    input_images = torch.stack([sam.preprocess(x["image"].cuda()) for x in batched_input], dim=0)
    image_embeddings = sam.image_encoder(input_images)
    sparse_embeddings, dense_embeddings = sam.prompt_encoder(points=None, boxes=box_xyxy.cuda(), masks=None)
    pred, _ = sam.mask_decoder(
        image_embeddings=image_embeddings,
        image_pe=sam.prompt_encoder.get_dense_pe(),
        sparse_prompt_embeddings=sparse_embeddings,
